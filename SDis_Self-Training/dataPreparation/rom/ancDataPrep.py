...
def ancData_rom(init_esm, init_ghs, process_ghs, year): 
    if year < 2000 :
    ...
    if init_esm == "yes":
        ...
        #Clip to extent
        outputfile = ancillary_path + "/{0}/ESM/{0}_ESM.tif".format(city)
        #gdalutils.vrt2tifGDAL(raster_file, esmFile, outputfile)
        dest_raster_path = ancillary_path + "/{0}/ESM/esm_residential_10.tif".format(city)
        
        # Select residential cells
        print("------------------------------ Splitting ESM raster to categories: Residential------------------------------")
        cmds = 'python {2}/gdal_calc.py -A "{0}" --A_band=1 --outfile="{1}" --calc="(A==255)" --NoDataValue=0'.format(outputfile, dest_raster_path, python_scripts_folder_path)
        print(cmds)
        #subprocess.call(cmds, shell=True)
        
        src = rasterio.open(dest_raster_path)
        ancdataset, rastergeo = osgu.readRaster(dest_raster_path)
        slice22 = src.read(1)
        x = np.nan_to_num(slice22, neginf=0, posinf=0) 
        destination_raster_path = ancillary_path + "/{0}/ESM/esm_residential_10A.tif".format(city)
        osgu.writeRaster(x[:,:], rastergeo, destination_raster_path)
    ...
    if init_ghs == "yes":
            ...
            ghsFiles= glob.glob(ancillary_path + "/euroData/GHS/{0}/2015/{0}/*.tif".format(city))
            ghsFile = ancillary_path + "/{0}/GHS/GHS_merged.tif".format(city)
            gdalutils.reprojectionGDAL(ghsFiles, ghsFile, projAlg)
            for projAlg in projAlgorithms:
                ...
                gdalutils.reprojectionGDAL(ghsFile, temp_outputfile, projAlg)
    ...
    if process_ghs == "yes":
        ...
        outputfile = ancillary_path + '/euroData/GHS/{0}_GHS_POP_100_V1_0_19_3_{0}_{1}.tif'.format(projAlg,resampleAlg, city)
        ...
        osgu.writeRaster(array[:,:], rastergeo, ancillary_path + "/{0}/GHS/{3}_GHS_POP_100_processed.tif".format(projAlg, resampleAlg, city, year))
