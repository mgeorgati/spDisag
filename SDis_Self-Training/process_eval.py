...
def process_eval(attr_value, city, key, methodopts, verMassPreserv, run_Evaluation, calc_Metrics, calc_Corr, 
    ...
    if run_Evaluation == "yes":       
        ...
        for i in attr_value:
            ...
            if city == 'cph':
                evalFiles = [ROOT_DIR + "/Results/{0}/apcnn/dissever01_RMSE_2018_{0}_Dasy_16unet_10epochspi_AIL3_it10_{1}.tif".format(city,i),
                    #ROOT_DIR + "/Results/{0}/apcnn/dissever01_CLF_2018_{0}_Dasy_16unet_10epochspi_AIL12_it10_{1}.tif".format(city,i),
                    ROOT_DIR + "/Results/{0}/apcnn/dissever01_RMSEflipped_2018_{0}_Dasy_16unet_10epochspi_AIL3_it10_{1}.tif".format(city,i),
                ...
                # Required files for plotting
                polyPath = ROOT_DIR + "/Shapefiles/{1}/{0}_{1}.shp".format(year,city)
                districtPath = ancillary_path + '/{0}/adm/{0}_west.geojson'.format(city)    
                waterPath = ancillary_path + '{0}/corine/waterComb_{0}_CLC_2012_2018.tif'.format(city)
            ...
            if plot_evalMaps== "yes":
                evalPath = ROOT_DIR + "/Evaluation/{0}_{1}/".format(city, 'ams') #date
                if not os.path.exists(evalPath):
                    createFolder(evalPath)
                    
                exportPathGT = ROOT_DIR + "/Evaluation/{0}_GT/{0}_{1}/".format(city, 'ams') #date
            ...
            for k in thres:
                ...
                if plot_evalMaps_knn == "yes":
                    evalPath = ROOT_DIR + "/Evaluation/{0}_knnGF/{0}_{1}/".format(city, 'knn_ams') #date
                    if not os.path.exists(evalPath):
                        createFolder(evalPath)
            ...
            if plot_Matrices == "yes" :
                for x in range(len(evalFiles)):
                    ...
                    print(path, fileName)
                    exportPathGT = ROOT_DIR + "/Evaluation/{0}_GT/{0}_{1}/".format(city, 'ams')
                    evalPath = ROOT_DIR + "/Evaluation/{0}_{1}/".format(city, 'ams') #date
                    evalPathKNN = ROOT_DIR + "/Evaluation/{0}_knn/{0}_{1}/".format(city, 'knn_ams')
                    evalPathKNNgf = ROOT_DIR + "/Evaluation/{0}_knnGF/{0}_{1}/".format(city, 'knn_ams')
                    evalPathMatrices = ROOT_DIR + "/Evaluation/{0}_{1}/".format(city, 'matrices', fileName)
