        ...
        for i in attr_value:
            ...
            if city == 'ams':
                evalFiles = [ROOT_DIR + "/Results/{0}/apcnn/dissever01_RMSE_2018_ams_Dasy_16unet_10epochspi_AIL12_it10_{1}.tif".format(city,i),
                    ...
                    ROOT_DIR + "/Results/{0}/apcnn/dissever01_RBLF_2018_ams_Dasy_16unet_10epochspi_12AIL12_it2_{1}.tif".format(city,i),
                    ROOT_DIR + "/Results/{0}/apcnn/dissever01_RBLF1_2018_ams_Dasy_16unet_10epochspi_12AIL12_it10_{1}.tif".format(city,i),
                ...
                districtPath = ancillary_path + '/{0}/adm/{0}_districts_south.geojson'.format(city)    
            ...
            if plot_evalMaps== "yes":
                evalPath = ROOT_DIR + "/Evaluation/{0}_{1}/".format(city, 'south') #date
                if not os.path.exists(evalPath):
                    createFolder(evalPath)
                    
                exportPathGT = ROOT_DIR + "/Evaluation/{0}_GT/{0}_{1}/".format(city, 'south') #date
            ...
            for k in thres:
                ...
                if plot_evalMaps_knn == "yes":
                    evalPath = ROOT_DIR + "/Evaluation/{0}_knnGF/{0}_{1}/".format(city, 'knn_south') #date
                    if not os.path.exists(evalPath):
                        createFolder(evalPath)
            ...
            if plot_Matrices == "yes" :
                for x in range(len(evalFiles)):
                    ...
                    print(path, fileName)
                    exportPathGT = ROOT_DIR + "/Evaluation/{0}_GT/{0}_{1}/".format(city, 'south')
                    evalPath = ROOT_DIR + "/Evaluation/{0}_{1}/".format(city, 'south') #date
                    evalPathKNN = ROOT_DIR + "/Evaluation/{0}_knn/{0}_{1}/".format(city, 'knn_south')
                    evalPathKNNgf = ROOT_DIR + "/Evaluation/{0}_knnGF/{0}_{1}/".format(city, 'knn_south')
                    evalPathMatrices = ROOT_DIR + "/Evaluation/{0}_{1}/south_{2}.png".format(city, 'matrices', fileName)
